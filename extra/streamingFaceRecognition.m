...
while double(get(fdrFig,'currentCharacter')) ~= 27 && ~isDone
	...
	for jj = 1:size(bboxes,1)%#ok
		if all(bboxes(jj,3:4) >= QC.minBBSize)
            ...
            h(jj) = text(bboxes(jj,1), bboxes(jj,2), bestGuess,'Color','red','FontSize',20);
            h(jj).Position(1:2) = bboxes(jj,1:2) - [0,20];
            h(jj).String = bestGuess;
